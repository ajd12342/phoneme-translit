...
def ipa2indic(tokenized_ipa):
    ...
    word_initial_positions = [0]+ [i+1 for i, x in enumerate(tokenized_ipa) if x == "<space>"]
    word_final_positions = [i-1 for i, x in enumerate(tokenized_ipa) if x == "<space>"]+[len(tokenized_ipa)-1]
